...
--  10. Aircraft_Seat_Map is now generated (one row per physical
--      seat) instead of seats being free-text. Six of the original
--      arbitrary seat labels (bookings 514, 517, 518, 520, 521, 528)
--      didn't correspond to real seats once an actual layout existed
--      (e.g. '33A' on an aircraft whose economy cabin only goes up to
--      row 31) and were corrected to valid seats on the same
--      aircraft — trg_booking_capacity_guard now validates every
--      Seat_Number against this table, so an invalid one would
--      reject the whole insert.
--  11. Real bug fix: Flight 1009 (STV -> BOM) used to reuse Route_ID
--      2, which Route actually defines as BOM -> AMD (the original
--      comment even called this "reuse route 1 reversed,
--      approximate"). Harmless for the static Route-graph journey
--      functions (they never touch Flight_Legs), but it would have
--      silently mislabeled Flight 1009's endpoints in the new
--      schedule-aware functions, which join Flight_Legs -> Route.
--      Added Route 21 (a real STV -> BOM route with its own
--      distance/fare) and repointed Flight_Legs/Booking to it.
-- ============================================================
...
-- ============================================================
-- 2c. AIRCRAFT_SEAT_MAP (generated — one row per physical seat)
-- ============================================================
-- Generated rather than hand-listed (would be 3000+ literal rows
-- otherwise). Layout: Business is 4-abreast (A,C,D,F — no middle
-- seat), rows 1..ceil(Tot_Bus_Seats/4); Economy is 6-abreast
-- (A-F), rows continuing right after the last business row through
-- ceil(Tot_Eco_Seats/6). The very first economy row on each
-- aircraft is marked as extra-legroom (a stand-in for an
-- over-wing exit row). Window/aisle/middle is derived from letter
-- position. Aircraft 112 (a freighter, 0/0 seats) gets no rows.

INSERT INTO Aircraft_Seat_Map (Aircraft_ID, Seat_Number, Seat_Type, Seat_Class_Features)
-- Business rows: 4-abreast, A/C/D/F
SELECT ac.Aircraft_ID,
       CONCAT(biz_row, biz_letter) AS Seat_Number,
       'Business',
       CASE WHEN biz_letter IN ('A','F') THEN 'Window' ELSE 'Aisle' END
FROM Aircraft ac
CROSS JOIN LATERAL generate_series(1, GREATEST(CEIL(ac.Tot_Bus_Seats / 4.0)::INT, 0)) AS biz_row
CROSS JOIN LATERAL unnest(ARRAY['A','C','D','F']) WITH ORDINALITY AS bl(biz_letter, biz_ord)
WHERE ac.Tot_Bus_Seats > 0
  AND (biz_row - 1) * 4 + biz_ord <= ac.Tot_Bus_Seats

UNION ALL

-- Economy rows: 6-abreast, A-F, continuing after the business cabin
SELECT ac.Aircraft_ID,
       CONCAT(business_rows + eco_row, eco_letter) AS Seat_Number,
       'Economy',
       CASE
           WHEN eco_row = 1 THEN 'Exit Row - Extra Legroom'
           WHEN eco_letter IN ('A','F') THEN 'Window'
           WHEN eco_letter IN ('C','D') THEN 'Aisle'
           ELSE 'Middle'
       END
FROM Aircraft ac
CROSS JOIN LATERAL (SELECT GREATEST(CEIL(ac.Tot_Bus_Seats / 4.0)::INT, 0) AS business_rows) br
CROSS JOIN LATERAL generate_series(1, GREATEST(CEIL(ac.Tot_Eco_Seats / 6.0)::INT, 0)) AS eco_row
CROSS JOIN LATERAL unnest(ARRAY['A','B','C','D','E','F']) WITH ORDINALITY AS el(eco_letter, eco_ord)
WHERE ac.Tot_Eco_Seats > 0
  AND (eco_row - 1) * 6 + eco_ord <= ac.Tot_Eco_Seats;
...
-- 6. ROUTE (21 records)
-- ============================================================
-- Distance/duration in km/minutes. Route 21 (STV -> BOM) was added
-- to fix a real bug found while building the schedule-aware journey
-- functions: Flight 1009 (STV -> BOM) originally reused Route_ID 2,
-- which this table actually defines as BOM -> AMD (the original seed
-- comment even flagged it as "reuse route 1 reversed, approximate").
-- That was harmless for the static Route-graph functions (which never
-- touch Flight_Legs), but would have silently mislabeled Flight
-- 1009's endpoints in any function that joins Flight_Legs -> Route —
-- exactly what the new schedule-aware functions do. Fixed by giving
-- STV -> BOM its own route instead of borrowing a mismatched one.
...
(21,  289.00,  50, 10, 2);   -- STV -> BOM (real distance/duration, not reused from Route 2)


-- ============================================================
-- 6b. ROUTE_FARE (42 records — Economy + Business per route)
-- ============================================================
...
(21, 'Economy', 3200.00), (21, 'Business', 8300.00);
...
-- Flight 1009: STV->BOM (direct, now correctly Route 21)
(1009, 21, 1, CURRENT_DATE - INTERVAL '10 days' + INTERVAL '7 hours',  CURRENT_DATE - INTERVAL '10 days' + INTERVAL '8 hours 10 minutes',
              CURRENT_DATE - INTERVAL '10 days' + INTERVAL '7 hours',  CURRENT_DATE - INTERVAL '10 days' + INTERVAL '8 hours 10 minutes', 'Landed'),
...
(513, 809, 1009, 21, 1, 'Economy', '7F',  3200.00,  'Confirmed'),
(514, 810, 1010, 11, 1, 'Business','2A',  16900.00, 'Confirmed'),
(515, 811, 1011, 12, 1, 'Economy', '25A', 5500.00,  'Confirmed'),
(516, 812, 1012, 17, 1, 'Economy', '10C', 4300.00,  'Confirmed'),
(517, 813, 1013, 12, 1, 'Economy', '12D', 5500.00,  'Confirmed'),
(518, 813, 1013, 18, 2, 'Economy', '12D', 6700.00,  'Confirmed'),
(519, 814, 1014, 18, 1, 'Economy', '20B', 6700.00,  'Confirmed'),
(520, 815, 1015, 14, 1, 'Economy', '9A',  4900.00,  'Confirmed'),
(521, 815, 1015, 18, 2, 'Economy', '9A',  6700.00,  'Confirmed'),
...
(528, 820, 1006, 10, 1, 'Economy', '30A', 6700.00,  'Confirmed'),
